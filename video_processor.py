    ...
    def _do_work_queue(self):
        ...
        while (not self._end_flag):
            try:
                ...
                rotated_roi = rotated_image[750:1100,200:800]

                if (not ret_val):
                    print("No image from video device, exiting")
                    break
                self._output_queue.put(rotated_roi, True, self._queue_put_wait_max)
            ...
            try:
                ...
                for one_network_proc in self._network_processor_list:
                    ...
                    if (not (self._end_flag)):
                        one_network_proc.start_aysnc_inference(rotated_roi)
                    else:
                        done=True
                        break
